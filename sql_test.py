...
db_connection_string = "postgresql://postgres:551@localhost:5432/QA"
db = create_engine(db_connection_string)

db_data = {
    "source": 'QA',
    "update_to": 'Quality Assurance',
    "rec-id": False
}
...
def select_1_row(connection, query, param):
    ...
    result = connection.execute(sql_statement, {"subject_id"})
...
def test_update():
    ...
    param = {}
    param['subject_title'] = db_data["update_data"]
    param['subject_id'] = id_update['subject_id']
    print(param)
    query = "UPDATE subject SET subject_title = :subject_title WHERE subject_id = :subject_id"
    insert_update(connection, query, param)
...
def test_delete():
    ...
    print(id_to_del)
    delete(connection, f"DELETE FROM subject WHERE subject_id = {id_to_del}")
